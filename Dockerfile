...
RUN curl -sSL --fail --retry 3 "http://dl.google.com/linux/chrome/deb/pool/main/g/google-chrome-stable/google-chrome-stable_105.0.5195.125-1_amd64.deb" -o google-chrome.deb &&\
    apt-get -y update &&\
    ...
    LATEST=`curl -sSL --fail --retry 3 "https://chromedriver.storage.googleapis.com/LATEST_RELEASE_105"` &&\
